(() => {
  ...
  window.lpInitContactIntroLines = function lpInitContactIntroLines() {
    ...
    const getTravelFactor = (el) => {
      if (el.classList.contains("lp-contactLine--w10")) return 0.55;
      if (el.classList.contains("lp-contactLine--w4")) return 0.72;
      return 0.88;
    };
    ...
    document.querySelectorAll(".lp-contactLines--top").forEach((svg) => {
      const lines = Array.from(svg.querySelectorAll(".lp-contactLine"));
      if (lines.length) {
    ...
    document.querySelectorAll(".lp-contactLines--bottom").forEach((svg) => {
      const lines = Array.from(svg.querySelectorAll(".lp-contactLine"));
      if (lines.length) {
        animateLineGroup(lines, {
          stagger: 0.22,
