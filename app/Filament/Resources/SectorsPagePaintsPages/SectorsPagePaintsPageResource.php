...
namespace App\Filament\Resources\SectorsPagePaintsPages;

use App\Filament\Resources\SectorsPagePaintsPages\Pages;
...
class SectorsPagePaintsPageResource extends Resource
{
    protected static ?string $model = SectorsPagePaintsPage::class;
            ...
            Section::make('الصورة الرئيسية')
                ...
                ->schema([
                    FileUpload::make('hero_image')
                        ...
                        ->image()
                        ->disk('public')
                        ->directory('site/sectors/sector-pages/paints')
                        ->visibility('public')
                        ->columnSpanFull(),
                ]),
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('article_ar')
                    ...
                    ->tooltip(fn (?SectorsPagePaintsPage $record): string => trim(strip_tags($record?->article_ar ?? ''))),

                TextColumn::make('article_en')
                    ->label('English')
                    ->formatStateUsing(fn (?string $state): string => Str::limit(trim(strip_tags($state ?? '')), 60))
                    ->tooltip(fn (?SectorsPagePaintsPage $record): string => trim(strip_tags($record?->article_en ?? ''))),
            ])
            ->recordUrl(fn (SectorsPagePaintsPage $record): string => static::getUrl('edit', ['record' => $record]))
            ->recordActions([
                EditAction::make(),
                DeleteAction::make(),
            ])
    ...
    {
        return [
            'index' => Pages\ListSectorsPagePaintsPages::route('/'),
            'create' => Pages\CreateSectorsPagePaintsPage::route('/create'),
            'edit' => Pages\EditSectorsPagePaintsPage::route('/{record}/edit'),
        ];
    }
