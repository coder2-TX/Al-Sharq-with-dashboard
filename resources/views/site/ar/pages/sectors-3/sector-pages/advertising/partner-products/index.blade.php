@php
    ...
    if ($partnerId > 0) {
        $partner = \App\Models\SectorsPageAdvertisingPartner::query()->find($partnerId);
    }

    if (!$partner && $partnerNameFromQuery !== '') {
        $partner = \App\Models\SectorsPageAdvertisingPartner::query()
            ->where('partner_name', $partnerNameFromQuery)
            ->first();
    }

    $partnerName = trim((string) ($partner?->partner_name ?: ($partnerNameFromQuery !== '' ? $partnerNameFromQuery : 'BrandSpark')));
...
<body
  class="lp-page--advertisingSector lp-page--medicalSector"
...
>
  ...
  <main id="advertising-partner-products-page">
    @include('site.ar.pages.sectors-3.sector-pages.advertising.partner-products.partials.section-1')
    @include('site.ar.pages.sectors-3.sector-pages.advertising.partner-products.partials.section-2')
  </main>
