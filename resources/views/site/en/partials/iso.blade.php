<section class="lp-section lp-iso" aria-label="ISO-9001:2015 Certificate">
  <div class="lp-iso__inner">
    <div class="lp-isoCard" role="group" aria-label="ISO-9001:2015">
      <span class="lp-isoCard__stroke" aria-hidden="true"></span>

      <div class="lp-isoCard__layer">
        <div class="lp-isoCard__row">
          <div class="lp-isoCard__main">
            <img
              class="lp-isoCard__icon"
              src="../assets/images/iso-9001.png"
              alt="ISO-9001:2015 certificate icon"
            />
            <span class="lp-isoCard__iso" dir="ltr" lang="en">ISO</span>
          </div>

          <p class="lp-isoCard__text">
            Al Sharq Company holds the
            <strong class="lp-enDigits lp-isoCard__cert" dir="ltr" lang="en">
              ISO-9001:2015
            </strong>
            certificate, confirming its commitment to a quality management
            system in accordance with international standards.
          </p>

          <a
            class="lp-cta lp-cta--iso"
            href="pages/iso/index.html"
            aria-label="ISO certificate details"
          >
            <span class="lp-cta__stroke" aria-hidden="true"></span>
            <span class="lp-cta__layer" aria-hidden="true">
              <span class="lp-cta__text">Details</span>
            </span>
          </a>
        </div>
      </div>
    </div>
  </div>
</section>